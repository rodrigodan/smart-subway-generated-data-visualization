...
// dataset: datas;
// xName: Geracao
// yObjs: Valores de avaliação em Y para cada combinação
// axisLables: {xAxis: 'Avaliações', yAxis: 'Gerações'}
...
function makeLineChart(dataset, xName, yObjs, axisLables) {
    ...
    chartObj.formatAsDecimal = d3.format(".5f");
    ...
    chartObj.bisectGeracao = d3.bisector(chartObj.xFunct).left; //< Can be overridden in definition
    ...
    // chartObj.yScale = d3.scale.linear().range([chartObj.height, 0]).domain([0.0000000082, 0.000000017]);
    // chartObj.yScale = d3.scale.log().base(Math.E).range([chartObj.height, 0]).domain([0.0000000083, d3.max(chartObj.yFuncts.map(chartObj.max))]);
    //#####
    chartObj.yScale = d3.scaleLog().base(Math.E).range([chartObj.height, 0]).domain([0.0000000083, d3.max(chartObj.yFuncts.map(chartObj.max))]);
    //#####
    chartObj.formatAsYear = d3.format("");

    //Create axis
    // chartObj.xAxis = d3.svg.axis().scale(chartObj.xScale).orient("bottom").ticks(30).tickFormat(chartObj.xFormatter); //< Can be overridden in definition
    //#####
    chartObj.xAxis = d3.axisBottom(chartObj.xScale).ticks(15).tickFormat(chartObj.xFormatter); //< Can be overridden in definition
    //#####
    // chartObj.yAxis = d3.svg.axis().scale(chartObj.yScale).orient("left").ticks(6).tickFormat(chartObj.yFormatter); //< Can be overridden in definition
    // chartObj.yAxis = d3.svg.axis().scale(chartObj.yScale).orient("left").ticks(6).tickFormat(function(d) { return (d * 1000000000).toFixed(2) + "E+9"; }); //< Can be overridden in definition
    //#####
    chartObj.yAxis = d3.axisLeft(chartObj.yScale).ticks(6).tickFormat(function(d) { return (d * 1000000000).toFixed(2) + "E+9"; }); //< Can be overridden in definition
    //#####
    ...
    function getYScaleFn(yObj) {
        return function (d) {
            console.log("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
            console.log(chartObj.yScale(yObjs[yObj].yFunct(d)));
...
// Render the chart
    chartObj.render = function () {
        ...
        // Draw Lines
        for (var y  in yObjs) {
            yObjs[y].path = chartObj.svg.append("path").datum(chartObj.data).attr("class", "line").attr("d", yObjs[y].line).style("stroke", color(y)).style("stroke-width", "3px").attr("data-series", y).on("mouseover", function () {
        ...
        .text("Geração")
        .attr("y", parseInt(chartObj.height) + 30)
        ...
        return chartObj;
        function mousemove() {
            var x0 = chartObj.xScale.invert(d3.mouse(this)[0]), i = chartObj.bisectGeracao(dataset, x0, 1), d0 = chartObj.data[i - 1], d1 = chartObj.data[i];
            try {
